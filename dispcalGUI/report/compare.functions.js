...
p.generate_report = function() {
	...
	this.report_html.push('		</tr>');
	this.report_html.push('		<tr>');
	this.report_html.push('			<th>&#160;</th><th>' + fields_match.join('</th><th>') + '</th><th>' + 'L*,a*,b*'.split(',').join('</th><th>') + '</th><th>&#160;</th><th>&#160;</th><th>' + 'L*,a*,b*'.split(',').join('</th><th>') + '</th><th>ΔL</th><th>ΔC</th><th>ΔH</th><th>ΔE</th><th>&#160;</th>');
	this.report_html.push('		</tr>');
	dupescount = 0;
	for (var i=0, n=0; i<this.data.length; i++) {
		dupes = this.data[i][fields_extract_indexes_i[o + 3] + 1];
		if (dupescount == dupes) {
			...
			var device = target.slice(fields_extract_indexes_i[0], fields_extract_indexes_i[o] + 1);
