...
from config import (autostart, autostart_home, btn_width_correction, 
	cmdfile_ext, confighome, runtimeconfig, datahome, defaults, enc, exe, 
	exe_ext, exedir, fs_enc, getbitmap, get_data_path, get_display, getcfg, 
	get_verified_path, iccprofiles, initcfg, isexe, profile_ext, resfiles, 
...
from worker import (Worker, check_argyll_bin, check_cal_isfile, 
	...
	check_set_argyll_bin, get_argyll_util, get_options_from_cprt,
	mac_terminal_do_script, mac_terminal_set_colors, 
	make_argyll_compatible_path, set_argyll_bin)
try:
...
pypath, pydir, pyname, pyext, isapp, runtype, build = runtimeconfig(__file__)
...
class DisplayCalibratorGUI(wx.Frame):
	...
	def init_menus(self):
		...
		menuitem.Enable(bool(len(self.worker.displays)))
		...
		menuitem.Enable(bool(len(self.worker.displays)) and bool(len(self.worker.instruments)))
	...
	def init_controls(self):
		...
		show_lut_ctrl = (len(self.worker.displays) > 1 and False in self.lut_access and True in self.worker.lut_access) or test
	...
	def update_displays(self):
		...
		self.display_ctrl.SetItems(self.worker.displays)
		display_ctrl_enable = bool(len(self.worker.displays))
		display_no = int(getcfg("display.number")) - 1
		self.display_ctrl.SetSelection(min(max(0, len(self.worker.displays) - 1), display_no))
		self.display_ctrl.Enable(display_ctrl_enable)
		if hasattr(self, "display_lut_ctrl"):
			self.display_lut_ctrl.Clear()
			i = 0
			for disp in self.worker.displays:
	...
	def update_comports(self):
		...
		self.comport_ctrl.SetSelection(min(max(0, len(self.worker.instruments) - 1), int(getcfg("comport.number")) - 1))
		self.comport_ctrl.Enable(bool(len(self.worker.instruments)))
	...
	def update_main_controls(self):
		...
		self.measurement_mode_ctrl.Enable(enable_cal and bool(len(self.worker.displays)) and bool(len(self.measurement_mode_ctrl.GetItems()) > 1))

		update_profile = self.profile_update_cb.GetValue()
		enable_profile = not(update_profile)

		self.calibrate_btn.Enable(bool(len(self.worker.displays)) and True in self.worker.lut_access and bool(len(self.worker.instruments)))
		self.calibrate_and_profile_btn.Enable(enable_profile and bool(len(self.worker.displays)) and True in self.worker.lut_access and bool(len(self.worker.instruments)))
		self.profile_btn.Enable(enable_profile and bool(len(self.worker.displays)) and bool(len(self.worker.instruments)))
	...
	def enable_spyder2_handler(self, event):
		if check_set_argyll_bin():
			...
			result = self.worker.exec_cmd(cmd, args, capture_output = True, skip_cmds = True, silent = True)
			if result:
				...
				if result == wx.ID_OK:
					if not os.path.exists(path):
						...
						return
					result = self.worker.exec_cmd(cmd, args + [path], capture_output = True, skip_cmds = True, silent = True)
					if result:
						InfoDialog(self, msg = lang.getstr("enable_spyder2_success"), ok = lang.getstr("ok"), bitmap = getbitmap("theme/icons/32x32/dialog-information"))
					else:
	...
	def check_overwrite(self, ext = ""):
		...
		dst_file = os.path.join(getcfg("profile.save_path"), getcfg("profile.name.expanded"), filename)
	...
	def calibrate(self, remove = False):
		...
		if result:
			cal = os.path.join(getcfg("profile.save_path"), getcfg("profile.name.expanded"), getcfg("profile.name.expanded") + ".cal")
			setcfg("last_cal_path", cal)
			self.previous_cal = getcfg("calibration.file")
			if self.profile_update_cb.GetValue():
				profile_path = os.path.join(getcfg("profile.save_path"), getcfg("profile.name.expanded"), getcfg("profile.name.expanded") + profile_ext)
				result = check_profile_isfile(profile_path, lang.getstr("error.profile.file_not_created"), parent = self)
				if result:
				...
				result = check_cal_isfile(cal, lang.getstr("error.calibration.file_not_created"), parent = self)
				if result:
					if self.install_cal:
	...
	def profile(self, apply_calibration = True, dst_path = None, skip_cmds = False, display_name = None):
		...
		if dst_path is None:
			dst_path = os.path.join(getcfg("profile.save_path"), getcfg("profile.name.expanded"), getcfg("profile.name.expanded") + profile_ext)
		cmd, args = self.worker.prepare_colprof(os.path.basename(os.path.splitext(dst_path)[0]), display_name)
		result = self.worker.exec_cmd(cmd, args, capture_output = "-as" in self.worker.options_colprof and self.worker.argyll_version <= [1, 0, 4], low_contrast = False, skip_cmds = skip_cmds)
		self.worker.wrapup(result, dst_path = dst_path)
		if "-as" in self.worker.options_colprof and self.worker.argyll_version <= [1, 0, 4]:
			_safe_print("".join(self.worker.output if result else self.worker.errors).strip())
			#safe_print(lang.getstr("success") if result else lang.getstr("aborted"))
		if result:
			try:
	...
	def install_profile(self, capture_output = False, cal = None, profile_path = None, install = True, skip_cmds = False, silent = False):
		...
		result = self.worker.exec_cmd(cmd, args, capture_output, low_contrast = False, skip_cmds = skip_cmds, silent = silent)
				...
				if "Installed" in line:
					if sys.platform == "darwin" and "-Sl" in args:
						...
						cmd, args = 'osascript', ['-e', 
							...
							get_display().split(",")[0]]
						result = self.worker.exec_cmd(cmd, args, capture_output = True, low_contrast = False, skip_cmds = True, silent = True)
					else:
						result = True
					break
			# if "-c" in args:
				...
				if sys.platform == "win32":
					...
					try:
						...
						if "-Sl" in args:
							...
							if not silent and \
								(not self.worker.exec_cmd("mkdir", ["-p", autostart], capture_output = True, low_contrast = False, skip_cmds = True, silent = True, asroot = True) or \
								not self.worker.exec_cmd("mv", ["-f", desktopfile_path, autostart], capture_output = True, low_contrast = False, skip_cmds = True, silent = True, asroot = True)):
		...
		self.worker.pwd = None # do not keep password in memory longer than necessary
		return result
	...
	def verify_calibration(self):
		...
		if self.worker.exec_cmd(cmd, args, capture_output = capture_output, skip_cmds = True):
			if capture_output:
				self.infoframe.Show()
	...
	def calibrate_and_profile_btn_handler(self, event):
		if sys.platform == "darwin" or debug: self.focus_handler(event)
		self.update_profile_name_timer.Stop()
		if check_set_argyll_bin() and self.check_overwrite(".cal") and self.check_overwrite(".ti3") and self.check_overwrite(profile_ext):
	...
	def calibrate_and_profile(self):
		...
		if self.worker.get_instrument_features().get("skip_sensor_cal") and self.worker.argyll_version >= [1, 1, 0]:
	...
	def start_profile_worker(self, success_msg, apply_calibration = True):
		self.worker.start(self.profile_finish, self.profile, ckwargs = {"success_msg": success_msg, "failure_msg": lang.getstr("profiling.incomplete")}, wkwargs = {"apply_calibration": apply_calibration }, progress_title = lang.getstr("create_profile"))
	...
	def profile_btn_handler(self, event):
		if sys.platform == "darwin" or debug: self.focus_handler(event)
		self.update_profile_name_timer.Stop()
		if check_set_argyll_bin() and self.check_overwrite(".ti3") and self.check_overwrite(profile_ext):
			cal = getcfg("calibration.file")
				...
				if ext.lower() in (".icc", ".icm"):
					...
					try:
						...
						if "cprt" in profile.tags: # get dispcal options if present
							self.worker.options_dispcal = ["-" + arg for arg in get_options_from_cprt(profile.tags.cprt)[0]]
	...
	def profile_finish(self, result, profile_path = None, success_msg = "", failure_msg = "", preview = True, skip_cmds = False):
		if result:
			...
			if profile_path:
				...
				profile_save_path = os.path.join(getcfg("profile.save_path"), getcfg("profile.name.expanded"), getcfg("profile.name.expanded"))
			...
			if ext.lower() in (".icc", ".icm"):
				...
				try:
					...
					if getcfg("calibration.file") != profile_path and "cprt" in profile.tags:
						options_dispcal, options_colprof = get_options_from_cprt(profile.tags.cprt)
			...
			if preview and has_cal: # show calibration preview checkbox
				...
				if ((sys.platform == "darwin" or (sys.platform != "win32" and self.worker.argyll_version >= [1, 1, 0])) and (os.geteuid() == 0 or get_sudo())) or \
	...
	def display_ctrl_handler(self, event):
		...
		if not hasattr(self, "display_lut_ctrl") or bool(int(getcfg("display_lut.link"))):
			if hasattr(self, "display_lut_ctrl"):
				self.display_lut_ctrl.SetStringSelection(self.worker.displays[display_no])
	...
	def display_lut_ctrl_handler(self, event):
		...
		try:
			i = self.worker.displays.index(self.display_lut_ctrl.GetStringSelection())
		except ValueError:
	...
	def display_lut_link_ctrl_handler(self, event, link = None):
		...
		if link:
			...
			try:
				lut_no = self.worker.displays.index(self.display_lut_ctrl.GetStringSelection())
			except ValueError:
				lut_no = self.display_ctrl.GetSelection()
	...
	def profile_save_path_btn_handler(self, event):
		...
		dlg = wx.DirDialog(self, lang.getstr("dialog.set_profile_save_path", getcfg("profile.name.expanded")), defaultPath = defaultPath)
		dlg.Center(wx.BOTH)
	...
	def create_profile_handler(self, event, path = None):
		...
		if path:
			...
			if result == wx.ID_OK:
				...
				if not tmp_working_dir or not check_create_dir(tmp_working_dir, parent = self): # check directory and in/output file(s)
					return
				# copy ti3 to temp dir
				ti3_tmp_path = make_argyll_compatible_path(os.path.join(tmp_working_dir, profile_name + ".ti3"))
				self.worker.options_dispcal = []
				self.worker.options_targen = []
				if debug: safe_print("Setting targen options:", self.worker.options_targen)
				display_name = None
				try:
					if source_ext.lower() == ".ti3":
				...
				if len(line) > 1:
					...
					if line[0] == "DEVICE_CLASS":
						...
						if measurement_mode in ("c", "l"):
							...
							self.worker.options_dispcal += ["-y" + measurement_mode]
						...
						if not lang.getstr("whitepoint") in settings:
							...
							self.worker.options_dispcal += ["-w%s,%s" % (getcfg("whitepoint.x"), getcfg("whitepoint.y"))]
							settings += [lang.getstr("whitepoint")]
						setcfg("calibration.luminance", stripzeros(round(Y * 100, 3)))
						self.worker.options_dispcal += ["-b%s" % getcfg("calibration.luminance")]
						...
						if value == "L_STAR":
							...
							try:
								...
								continue
						self.worker.options_dispcal += ["-" + getcfg("trc.type") + getcfg("trc")]
						settings += [lang.getstr("trc")]
					elif line[0] == "DEGREE_OF_BLACK_OUTPUT_OFFSET":
						setcfg("calibration.black_output_offset", stripzeros(value))
						self.worker.options_dispcal += ["-f%s" % getcfg("calibration.black_output_offset")]
						...
						self.worker.options_dispcal += ["-k%s" % getcfg("calibration.black_point_correction")]
						...
						self.worker.options_dispcal += ["-B%s" % getcfg("calibration.black_luminance")]
						settings += [lang.getstr("calibration.black_luminance")]
					elif line[0] == "QUALITY":
						setcfg("calibration.quality", value.lower()[0])
						self.worker.options_dispcal += ["-q" + getcfg("calibration.quality")]
	...
	def aboutdialog_handler(self, event):
		...
		items += [wx.StaticText(self.aboutdialog, -1, "")]
		items += [wx.StaticText(self.aboutdialog, -1, u"Argyll CMS %s © Graeme Gill" % self.worker.argyll_version_string)]
