...
from config import btn_width_correction, defaults, getbitmap, getcfg, get_verified_path, setcfg
...
class ConfirmDialog(BaseInteractiveDialog):
	...
	def __init__(self, oparent = None, id = -1, title = appname, msg = "", 
	   ...
	   size = (400, -1)):
		BaseInteractiveDialog.__init__(self, oparent, id, title, msg, ok, bitmap, pos, size, show = False, logit = False)
...
class InfoDialog(BaseInteractiveDialog):
	...
	def __init__(self, oparent = None, id = -1, title = appname, msg = "", 
	   ok = "OK", bitmap = None, pos = (-1, -1), size = (400, -1), 
	   show = True, logit = True):
		BaseInteractiveDialog.__init__(self, oparent, id, title, msg, ok, bitmap, pos, size, show, logit)
...
class LogWindow(InvincibleFrame):
	...
	def __init__(self, parent = None, id = -1):
		InvincibleFrame.__init__(self, parent, id, lang.getstr("infoframe.title"), pos = (int(getcfg("position.info.x")), int(getcfg("position.info.y"))), style = wx.DEFAULT_FRAME_STYLE | wx.FRAME_TOOL_WINDOW)
		self.last_visible = False
		self.panel = wx.Panel(self, -1)
		self.sizer = wx.BoxSizer(wx.VERTICAL)
		self.panel.SetSizer(self.sizer)
		self.log_txt = wx.TextCtrl(self.panel, -1, "", style = wx.TE_MULTILINE | wx.TE_READONLY)
		...
		self.save_as_btn = wx.BitmapButton(self.panel, -1, getbitmap("theme/icons/16x16/media-floppy"), style = wx.NO_BORDER)
		...
		self.clear_btn = wx.BitmapButton(self.panel, -1, getbitmap("theme/icons/16x16/edit-delete"), style = wx.NO_BORDER)
		...
		self.SetSaneGeometry(int(getcfg("position.info.x")), int(getcfg("position.info.y")), int(getcfg("size.info.w")), int(getcfg("size.info.h")))
	...
	def OnSaveAs(self, event):
		...
		dlg = wx.FileDialog(self, lang.getstr("save_as"), defaultDir = defaultDir, defaultFile = defaultFile, wildcard = lang.getstr("filetype.log") + "|*.log", style = wx.SAVE | wx.OVERWRITE_PROMPT)
		dlg.Center(wx.BOTH)
