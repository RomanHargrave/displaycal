...
which 0launch.exe > nul && (
	...
	0launch http://0install.net/2006/interfaces/0publish --add-version=%version% --archive-url=http://dispcalgui.hoech.net/archive/%version%/dispcalGUI-%version%.tar.gz --archive-file=dist\dispcalGUI-%version%.tar.gz --set-main="dispcalGUI-%version%/dispcalGUI.pyw" --set-released="%released%" --set-stability=stable -x dist\0install\dispcalGUI.xml
)
