...
/usr/bin/python2.5 setup.py bdist_rpm --cfg=$dist-argyll --skip-instrument-configuration-files --use-distutils 2>&1 | tee dispcalGUI-$version-py2.5-$dist-argyll.$platform.bdist_rpm.log
mv -f dist/dispcalGUI-$version-1.$platform.rpm dist/dispcalGUI-$version-py2.5-$dist-argyll-1.$platform.rpm
